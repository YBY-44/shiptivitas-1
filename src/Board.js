  ...
  componentDidMount() {
    ...
    .on('drag', (el) => {
        if (this.swimlanes.backlog.current.contains(el)) {
          el.className = el.className.replace(' Card-grey', '');
        } else if (this.swimlanes.inProgress.current.contains(el)) {
          el.className = el.className.replace(' Card-blue', '');
        } else if (this.swimlanes.complete.current.contains(el)) {
          el.className = el.className.replace(' Card-green', '');
        }
      })
    .on('drop', (el) => {
        if (this.swimlanes.backlog.current.contains(el)) {
    ...
    return () => {
      this.drake.destroy(); // 清理 Dragula 实例以避免内存泄漏
    };
  }
  render() {
    return (
